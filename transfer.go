...
//
//func transferRecord(state map[string]interface{}, predicate toolbox.Predicate, dataTypeProvider func() interface{}, encoded []byte, transformer Transformer, transfer *Transfer, transformedTargets map[bytes.Buffer]*TargetTransformation, task *TransferTask, decodingError *decodingError, contentEnricher ContentEnricher, request *TransferObjectRequest) error {
...
func transferByteRecord(state map[string]interface{}, predicate toolbox.Predicate, dataTypeProvider func() interface{}, encoded []byte, transformer Transformer, transfer *Transfer, transformedTargets map[string]*TargetTransformation, task *TransferTask, decodingError *decodingError, contentEnricher ContentEnricher, request *TransferObjectRequest) error {
	record := dataTypeProvider()
	err := decodeJSONTarget(encoded, record)
	if err != nil {
		decodingError.count++
	...
	if predicate == nil || predicate.Apply(record) {
		...
		transformedObject, _ := target.(bytes.Buffer)
		//err = encodeJSONSource(buf, target)
		//if err != nil {
		//	return err
		//}
		...
		if !found {
			...
			transformedTargets[targetKey] = &TargetTransformation{
				targetRecords: make([]bytes.Buffer, 0),
				ProcessedTransfer: &ProcessedTransfer{
					Transfer: targetTransfer,
				},
			}
